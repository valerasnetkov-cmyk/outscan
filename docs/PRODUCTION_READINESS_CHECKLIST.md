# OUTSCAN Production Readiness Checklist
...
| ID | Level | Criterion | Minimum evidence |
|---|---|---|---|
| PR-SCN-001 | BLOCKER | Scanner workers do not have direct credentials/access to the primary application DB unless explicitly justified by architecture. | Runtime/config evidence |
...
| PR-SCN-004 | BLOCKER | Worker secrets are least-privilege and scoped to required engines/providers only. | Secret/config review |
...
| PR-ABU-006 | REQUIRED | Administrative trusted grants remain bound to an explicit allowed target and are auditable. | Grant negative tests |
...
Before unrestricted public Guest Scan launch, these criteria cannot be `WARN` or `N/A`:
