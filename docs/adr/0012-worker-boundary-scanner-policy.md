...
The fixed process adapter and minimal first-party Guest scanner are now implemented. Scanner tests use injected DNS/transport doubles to prove exact policy/capability enforcement, built-in and deployment-specific full-set destination denial, a shared DNS+HTTP request budget, conservative security-header decisions, bounded same-host RFC 9116 `security.txt` validation, target-safe projection and canonical output; no live DNS or network operation occurs. Production container/OS isolation, enforced egress, managed-secret providers and owned-target live TLS evidence remain required before Gate B1.
