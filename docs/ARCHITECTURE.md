...
Current-version deployment targets Ubuntu + Docker Compose, accepted by the owner on 2026-09-10. B1 implementation must establish scanner isolation and enforced egress on that target. A later Kubernetes migration concerns scanner execution workers; it does not require moving the whole platform now or changing verification, scanner policy, lease/fence or result contracts. See [deployment target](DEPLOYMENT_TARGET.md).
...
- `apps/api/src/guest-session`: shared strict snapshot and server-owned bootstrap composition over fresh bounded active/retained MAC keys plus an injected scope-revocation provider. Bootstrap reuses an authenticated non-revoked session without extending expiry or issues a new active-key scope; scan creation consumes the same fresh-read provider so retained-key removal immediately denies admission before state change. PostgreSQL supplies revocation and a hardened mounted-file keyring provider is available for deployment composition, while managed key provisioning and route registration remain absent.
...
- `apps/api/src/guest-http`: detached Fastify session-bootstrap, scan-creation and result plugins. Bootstrap requires an exact configured HTTPS Origin, no query/body, a server-owned reuse/issue decision and a canonical Set-Cookie value. Creation accepts only bounded JSON and single sensitive headers, projects the closed status/error schema and bounds `Retry-After`; result access has closed 400/404/503 mappings, mandatory privacy headers, no implicit HEAD and no token-in-query path. `buildApp()` deliberately registers none before Gate B1, so they are not public route evidence.
...
- `apps/api/src/guest-persistence`: concrete SQL-first Guest idempotency, abuse, session revocation, attempt-lease, cancellation, terminal-result, rejection-event, result-read and retention adapters. Revocation stores only a scope digest for exactly 24 hours and supports bounded expiry pruning; scan creation awaits it and fails unavailable on ambiguity. Rejection events contain only closed code/classification plus current scan/attempt/fence, are idempotent/append-only and cascade with Guest retention. `apps/api/src/guest-retention` serializes bounded cleanup cycles and persists separate minimized PLATFORM run/alert telemetry with 30-day expiry. SERIALIZABLE lease CAS and terminal transitions remain PostgreSQL authority; production worker deployment and HTTP routes remain absent.
...
The Guest database schema, core repositories/transactions, internal creation composition, BullMQ/supervisor lease flow and a minimal scanner executable are implemented. Durable minimized queue telemetry has an internal runtime, PostgreSQL store, worker lifecycle scheduling and a dedicated process entrypoint, but the worker and alert exporter are not deployed. Guest session/create/result route adapters exist only as detached contract-tested plugins; production registration, worker/container/egress deployment and owned-target live socket/TLS evidence are not implemented or exposed yet.
