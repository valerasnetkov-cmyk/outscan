...
The validator is consumed by the trusted supervisor pre-launch and GUEST_SAFE orchestration boundaries in `apps/api/src/supervisor` and is independently rechecked by the minimal first-party scanner in `apps/api/src/guest-scanner`. Authorized capabilities, budgets and artifact identity are immutable snapshots before launch. The scanner currently implements resolution, pinned HTTPS/TLS, conservative HSTS/CSP/frame/nosniff/referrer/permissions posture checks and a bounded unsigned RFC 9116 `/.well-known/security.txt` subset; it shares one request budget across DNS and HTTP, applies the configured internal CIDR deny-list on every resolution and marks the remaining detector groups unavailable. `security.txt` uses only HTTPS and same-host redirects, requires plain UTF-8, Contact, a future Expires value and a matching Canonical URI when present, and never projects file content. These checks intentionally accept a narrow useful subset rather than claiming full browser-standard parsing. No public route or production-isolated scanner deployment is exposed yet.
